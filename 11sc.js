...
// на старт
// запускает функция setInterval(function () { фукнция добавлять  +1 секунду }, 1000)
// остановить интервал с помощью clearSetInterval();
...
editeTaskParrent.addEventListener('click', doneTaskKlickAndClickCheck);
...
closePopup.addEventListener('click', function (e) {
	...
	showPopup.classList.remove('js-visibility-appear');
	showPopup.classList.add('js-visibility-hide');
});

//___________________________________________________

function doneTaskKlickAndClickCheck(e) {
...
function clearCheckedTasks() {
	...
	if (!activeTask) {
		...
		clearOnlyCheckedBtn.classList.remove();
	}
...
colorBtn.addEventListener('click', () => {
	...
	if (body.classList.contains('js-color__content')) {
...
//проверка цвета заднего фона при добавлении новой карточки
inputBtn.addEventListener('click', () => {
	// получаем ссылку на элемент карточки
	const taskCard = document.querySelector('.js-tasks__active');
	// получаем ссылку на элемент обертки
	const wrapper = document.querySelector('.wrapper');

	// проверяем, есть ли у обертки класс js-color__content
	if (wrapper.classList.contains('js-color__content')) {
		// если есть, добавляем класс task__style-dark к карточке
		taskCard.classList.add('task__style-dark');
	} else {
		// если нет, удаляем класс task__style-dark из карточки
		taskCard.classList.remove('task__style-dark');
	}
});

//проверка на наличие карточек и совершаем измненение цвета в зависимости от состояния чекбокса
